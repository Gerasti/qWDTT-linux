...
- `-bl-file PATH` / `--black-list-file` - прочитать домены из JSON-файла (поле `bypassRoutes`), можно комбинировать с `-bl`
  ...
  - **Cross-mode синхронизация kernel-роутов:** если `tun`/`raw`‑профиль подключается *после* уже запущенных `socks`‑профилей с bypass‑списками, их домены автоматически добавляются в kernel host‑роуты (через `127.0.0.1` control-socket, read-only `GET_BL`), чтобы соединения из socks‑daemon не «протекали» в `wg-qwdtt`/`raw-qwdtt`. Требований root у socks‑профиля при этом нет — запрос инициирует kernel‑профиль (где уже есть `cap_net_admin`).
  - **Известное ограничение (не исправлено):** hot‑reload bypass‑списка уже запущенного `socks`‑профиля (`bl load -r` / `-bl ... -r`) **не** пересинхронизирует kernel‑роуты уже поднятого `tun`/`raw`‑профиля — изменения вступят в силу после **переподключения** kernel‑профиля. До переподключения поведение может отличаться от ожидаемого (direct‑трафик socks не будет обходить kernel‑туннель).
...
- `-r` / `--reload` — применить изменения на лету (hot-reload) без переподключения.
  - **Важно:** для `socks`‑профиля hot‑reload меняет лишь его users‑level bypass‑список; если параллельно запущен `tun`/`raw`‑профиль, kernel‑роуты к его доменам будут пересинхронизированы только при следующем connect kernel‑профиля (см. known limitation выше).
