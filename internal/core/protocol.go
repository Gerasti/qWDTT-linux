...
// SendAuth отправляет авторизацию на сервер (для воркеров, которые не запрашивают конфиг)
...
func RequestRawConfig(conn net.Conn, deviceID, password string) (ip, dnsCSV string, mtu int, err error) {
	...
	mtu, convErr := strconv.Atoi(parts[2])
	if convErr != nil {
		return "", "", 0, fmt.Errorf("некорректный MTU в RAWCONF: %q", parts[2])
	}
	return strings.TrimSpace(parts[0]), strings.TrimSpace(parts[1]), mtu, nil
}
