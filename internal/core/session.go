...
//
// RTP-obfs (ChaCha20-Poly1305/AEAD, obfs.go) уже даёт полноценное
...
// отдельного хендшейка на каждую из 9 воркер-сессий. Используется только
// когда tp.NoDTLS=true И useWrap=true (иначе тут вообще нет шифрования —
// тогда обязателен DTLS, см. ветку ниже).
...
func RunSession(
	...
	// горутины воркера) — не более одной новой TURN-аллокации за тик по
	// всей группе. Без этого стаггеринг старта не спасает: на нестабильной
	// сети (ретраи/задержки Allocate) несколько воркеров всё равно
	// накладываются друг на друга и вместе выжигают VK-квоту (error 486).
	if allocateGate != nil {
		select {
		case <-allocateGate:
		case <-ctx.Done():
	...
	var relayWg sync.WaitGroup
 	...
 	if useWrap && (tp.NoDTLS || tp.RawMode) {
		...
		if useWrap {
			var obfsErr error
			dtlsObfsCfg, obfsErr = NewObfsConfig(tp.ObfsMode)
			if obfsErr != nil {
				return false, fmt.Errorf("RTP-obfs config: %w", obfsErr)
			}
			obfsWriteState, obfsErr = NewObfsState()
	...
	if getConfig && configCh != nil && tp.RawMode {
		...
		// Воркеры, которые не запрашивают конфиг, отправляют AUTH
	...
	if tp.RawMode {
		...
		// rawRouter — иначе "мёртвые" соединения засоряют round-robin при
		// быстром переподключении того же устройства.
		go func() {
			select {
			case <-ctx.Done():
	...
	// Пакет не пишется напрямую в activeConn — кладётся в slot.PrioCh
	// неблокирующе, тем же путём, что и мелкие ACK-пакеты, и уходит через
	// единственную writer-горутину.
	go func() {
		defer proxyWg.Done()
	...
	go func() {
		defer proxyWg.Done()
		defer sessCancel()
		// Drain stale PrioCh packets on exit so keepalive goroutine doesn't block.
		defer func() {
			for {
				select {
				case p := <-slot.PrioCh:
		...
		for {
			...
			// 3с, не sessionReadTimeout (30 мин). Запись пишет в UDP-сокет
			// к TURN relay, а не читает долгоживущее соединение; 30-минутный
			// дедлайн означал бы, что зависшая запись держит Writer колом
			// полчаса, пока очередь SendCh/PrioCh копится.
	...
	go func() {
		...
		for {
			...
			// (25-44 байта). WireGuard-пакеты всегда начинаются с байта
