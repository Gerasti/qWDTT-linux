...
func workerErrorHint(err error) string {
	...
	case strings.Contains(text, "wrap_auth_timeout"):
		return "сервер не ответил на WRAP/DTLS — проверьте пароль, Device ID, IP/порт и что wdtt-server запущен"
	...
	case strings.Contains(text, "lookup") || strings.Contains(text, "no such host"):
		return "DNS не резолвит адрес — смените DNS в настройках"
