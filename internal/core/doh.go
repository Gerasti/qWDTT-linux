	...
	if strings.HasPrefix(arg, "doh:") {
		...
		for _, part := range strings.Split(raw, ",") {
			...
			if ep, ok := normalizeDoHEndpoint(part); ok {
				...
				log.Printf("[CLIENT] DNS: invalid DoH endpoint %q, skipped", part)
			}
		}
		if len(out) > 0 {
			return out
		}
		log.Printf("[CLIENT] DNS: no valid DoH endpoints in %q, using doh-yandex as fallback", original)
		return goDoHEndpointsForPreset("doh-yandex")
	}
	knownDoHPresets := map[string]bool{
		"doh-yandex":     true,
		"doh-cloudflare": true,
		"doh-google":     true,
	}
	normalized := strings.ToLower(arg)
	if !knownDoHPresets[normalized] {
		log.Printf("[CLIENT] DNS: unknown DoH preset %q, using doh-yandex as fallback", original)
		return goDoHEndpointsForPreset("doh-yandex")
	}
